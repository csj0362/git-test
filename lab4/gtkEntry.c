...
void input_completed(GtkWidget *entry, gpointer data){
	...
	if(strcmp(text, password)==0)
		gtk_label_set_text(GTK_LABEL((GtkWidget *) data), "login access\n");
	else
		gtk_label_set_text(GTK_LABEL((GtkWidget *) data), "login denied\n");
}
...
int main(int argc, char *argv[]){
	...
	gtk_window_set_title(GTK_WINDOW(window), "input");
	...
	message_label = gtk_label_new("name and passwd");
